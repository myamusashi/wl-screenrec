...
struct ToplevelSamplingState<S> {
    cap: S,
    what_to_enc: WhatToEncode,
    history_already_triggered: bool,
    selected_format: DmabufFormat,
    dri_device: PathBuf,
    _hw_device_ctx: AvHwDevCtx,
    frames_rgb: AvHwFrameCtx,
    sampler: FrameCadenceSampler,
}
...
enum EncConstructionStage<S> {
    ...
    Sampling(ToplevelSamplingState<S>),
...
impl<S> EncConstructionStage<S> {
    ...
    fn unwrap_cap(&mut self) -> &mut S {
        match self {
            ...
            EncConstructionStage::Sampling(sampling) => &mut sampling.cap,
    ...
    fn on_sigusr1(&mut self) {
        match self {
            ...
            EncConstructionStage::Sampling(sampling) => sampling.history_already_triggered = true,
...
impl<S: CaptureSource + 'static> Dispatch<WlRegistry, GlobalListContents> for State<S> {
    fn event(
        ...
        match event {
            ...
            _ => {
                debug!("unexpected WlRegistry event: {event:?}");
            }
        }
    }
}
...
impl<S: CaptureSource + 'static> State<S> {
    ...
    fn on_frame_allocd(&mut self, qhandle: &QueueHandle<State<S>>, frame: &S::Frame) {
        ...
        match &mut self.enc {
            ...
            EncConstructionStage::Sampling(_) => {
                self.queue_sampling_capture(qhandle);
            }
    ...
    fn queue_sampling_capture(&mut self, qhandle: &QueueHandle<Self>) {
        let EncConstructionStage::Sampling(sampling) = &mut self.enc else {
            unreachable!("sampling capture queued without sampling state");
        };
        let InFlightSurface::Allocd(frame) = &self.in_flight_surface else {
            panic!("queue_sampling_capture called in a strange state");
        };
        let mut av_surface = sampling.frames_rgb.alloc().unwrap();
        av_surface.set_color_space(ffmpeg::color::Space::RGB);
        let (desc, av_mapping) = map_drm(&av_surface);
        assert_eq!(desc.nb_layers, 1);
        ...
        let wl_buffer = wl_buffer_params.create_immed(
            sampling.selected_format.width,
            sampling.selected_format.height,
            sampling.selected_format.fourcc as u32,
            zwp_linux_buffer_params_v1::Flags::empty(),
            qhandle,
            (),
        );
        sampling.cap.queue_copy(
            self.args.damage,
            &wl_buffer,
            (
                sampling.selected_format.width,
    ...
    fn on_sampling_complete(
        ...
        let pts_abs = ((i64::from(tv_sec_hi) << 32) + i64::from(tv_sec_lo)) * 1_000_000_000
            + i64::from(tv_nsec);
        let InFlightSurface::CopyQueued {
            av_surface,
            av_mapping,
            wl_frame,
            wl_buffer,
            ...
            panic!("on_sampling_complete called in a strange state");
        };
        drop(av_mapping);
        drop(av_surface);
        wl_buffer.destroy();

        let (ready, measured, used_wallclock) = match &mut self.enc {
            EncConstructionStage::Sampling(sampling) => {
                sampling.cap.on_done_with_frame(wl_frame);
                sampling.sampler.add_wallclock(Instant::now());
                sampling.sampler.add_timestamp(pts_abs);
                (
                    sampling.sampler.is_ready(),
                    sampling.sampler.framerate(),
                    sampling.sampler.used_wallclock(),
                )
            }
            _ => unreachable!("sampling completion without sampling state"),
        };
        ...
        if used_wallclock {
            warn!(
                "toplevel presentation timestamps were degenerate; using measured wall-clock cadence"
            );
        }
        let framerate = measured.unwrap_or_else(|| {
            warn!(
                "toplevel cadence samples were degenerate; using 60 fps fallback for rate control"
            );
            Rational::new(60, 1)
        });
        info!("measured toplevel capture cadence: {framerate:?}");

        let EncConstructionStage::Sampling(sampling) =
            mem::replace(&mut self.enc, EncConstructionStage::Intermediate)
        else {
            unreachable!("sampling state disappeared before encoder construction");
        };
        let enc = match EncState::new(
            &self.args,
            sampling.selected_format,
            Some(framerate),
            sampling.what_to_enc,
            sampling.history_already_triggered,
            &sampling.dri_device,
        ) {
            Ok(enc) => enc,
            Err(error) => {
                error!("failed to create encoder after cadence sampling: {error:#}");
                self.errored = true;
                return;
            }
        };
    ...
    fn on_copy_complete(
        ...
        if matches!(self.enc, EncConstructionStage::Sampling(_)) {
            self.on_sampling_complete(qhandle, tv_sec_hi, tv_sec_lo, tv_nsec);
            return;
        }
        let CompleteState { enc, cap, .. } = self.enc.unwrap();
    ...
    fn on_copy_fail(&mut self, reason: CopyFailReason, qhandle: &QueueHandle<Self>) {
        if matches!(self.enc, EncConstructionStage::Sampling(_)) {
            ...
            } = self.in_flight_surface.take()
            {
                ...
                drop(av_surface);
                wl_buffer.destroy();
                if let EncConstructionStage::Sampling(sampling) = &self.enc {
                    sampling.cap.on_done_with_frame(wl_frame);
                }
            }
            ...
            warn!(
                "toplevel cadence sampling failed after {} samples ({reason:?}); using 60 fps fallback",
                match &self.enc {
                    EncConstructionStage::Sampling(sampling) => sampling.sampler.sample_count(),
                    _ => 0,
                }
            );
            let EncConstructionStage::Sampling(sampling) =
                ...
                unreachable!("sampling state disappeared after capture failure");
            };
            let enc = match EncState::new(
                &self.args,
                sampling.selected_format,
                ...
                Ok(enc) => enc,
                Err(error) => {
                    error!("failed to create encoder after sampling failure: {error:#}");
                    self.errored = true;
                    return;
                }
            };
        ...
        if let WhatToEncode::OutputRoi(output, _) = &enc.enc {
            ...
            warn!(
                "copy failed for toplevel capture (reason: {reason:?}), trying to capture a new frame..."
            );
            self.queue_alloc_frame(qhandle);
        }
    }
    ...
    fn negotiate_format(
        ...
        match mem::replace(&mut self.enc, EncConstructionStage::Intermediate) {
            EncConstructionStage::EverythingButFormat {
                ...
                if matches!(output, WhatToEncode::Toplevel(_)) && self.args.max_fps.is_none() {
                    let (hw_device_ctx, frames_rgb) =
                        match create_capture_context(&self.args, &selected_format, dri_device) {
                            Ok(context) => context,
                            Err(error) => {
                                error!("failed to prepare toplevel framerate sampler: {error:#}");
                                self.errored = true;
                                return;
                            }
                        };
                    info!("sampling toplevel presentation cadence before opening encoder");
                    self.enc = EncConstructionStage::Sampling(ToplevelSamplingState {
                        cap,
                        what_to_enc: output,
                        history_already_triggered,
                        selected_format,
                        dri_device: dri_device.to_path_buf(),
                        _hw_device_ctx: hw_device_ctx,
                        frames_rgb,
                        sampler: FrameCadenceSampler::new(),
                    });
                    match self.in_flight_surface {
                        InFlightSurface::None => self.queue_alloc_frame(eq),
                        InFlightSurface::Allocd(_) => self.queue_sampling_capture(eq),
                ...
                let enc = match EncState::new(
                    ...
                    match &output {
                        ...
                        WhatToEncode::Toplevel(_) => {
                            self.args.max_fps.map(|fps| Rational::new(fps as i32, 1))
                        }
                    },
                    output.clone(),
                    history_already_triggered,
...
fn create_capture_context(
    ...
    let mut hw_device_ctx = if args.vulkan {
        ...
        AvHwDevCtx::new_libva(dri_device).map_err(|e| {
            anyhow!("Failed to load vaapi device: {e}. This is likely *not* a bug in wl-screenrec.")
        })?
    };
    let frames_rgb = hw_device_ctx
        .create_frame_ctx(
            dmabuf_to_av(capture_format.fourcc),
        ...
        .with_context(|| {
            format!("Failed to create capture frame context for format {capture_format:?}")
        })?;
    Ok((hw_device_ctx, frames_rgb))
}
